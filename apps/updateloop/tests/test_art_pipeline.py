...
def test_animated_kv_keeps_poster_and_indexes_every_png(tmp_path: Path):
    ...
    assert [(art.category, art.id) for art in manifest.arts] == [
        ...
        ("image", "act3mainss_01/effect"),
        ("image", "act3mainss_01/kv_bg"),
    ]
    assert manifest.arts[0].image.content == expected
...
def test_audio_media_round_trips_through_the_cache(tmp_path: Path):
    ...
    audio_path.with_suffix(".wav.audio.json").write_text('{"duration":2.5}', encoding="utf-8")
    ...
    write_art_manifest(manifest, cached_root)
    cached = read_art_manifest(cached_root)
